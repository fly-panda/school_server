<template>
	<div>
		<div class="previewContents">
			...
			<div class="content">
				<div class="discribe" v-html="previewObj.describe"></div>
				<div v-for="(cont,index) in previewObj.data" :key="index">
							...
							<div class="view-list">
								...
								<div>
									...
									<div class="cont">
										<!-- {{studentList}} -->
										<ul class="peo-list">
											<li v-for="(item,index) in studentList" :key="index" @click="selClick(item,index)">
												<span class="name-cls">{{item.name}}</span>
												<span
													class="check-cls"
													:class="{'active-cls':item.userid==studentObj.userid}"
												></span>
											</li>
										</ul>
									</div>
									...
									<div class="cont">
										...
										<ul class="peo-list">
											...
											>
												...
												<span
													class="check-cls"
													:class="{'active-cls':item.userid==teacherObj.userid}"
												></span>
											</li>
										</ul>
									</div>
										...
										<div>
											...
											<RadioGroup v-model="cont.obj.value">
												<Row v-for="(item,ind) in cont.obj.items" :key="ind">
													<Radio class="checkboxItem" :label="item.departid">
					...
					<div class="selectStudentContainer" v-if="cont.ele=='download'">
						...
						<div
							class="downloadBtn"
							v-for="(item,ind) in cont.obj.items"
							:key="ind"
					...
					<div class="imgPreview selectStudentContainer" v-if="cont.ele=='imgshow'">
						<p v-for="(item,ind) in cont.obj.imgArr" :key="ind">
					...
					<div class="selectStudentContainer" v-if="cont.ele=='uploadimg'">
						...
						<div class="imgPreview uploadimg-cls">
							<p v-for="(item,ind) in cont.obj.uploadList" :key="ind">
								<img :src="baseImg+item.url" alt>
								<Icon type="md-close-circle" class="delImg" @click="delImgFun(cont.obj,ind)"/>
							</p>
							<p class="post">
								<Icon type="md-camera"/>上传图片
								<input
									class="files"
									:id="'fileImg'+index"
									type="file"
									accept="image/*"
									@change="addFileImg(cont.obj,index)"
								>
							</p>
						</div>
					...
					<div class="selectStudentContainer" v-if="cont.ele=='selectcontact'">
						...
						<div>
							<Select
								...
								@on-change="changeSel(cont.obj,cont.obj.value)"
							>
								<Option
									v-for="(item,ind) in cont.obj.items"
									:value="ind"
					...
					<div class="selectStudentContainer" v-if="cont.ele=='score'">
						...
						<!--                 {{cont.obj.valueArr}} -->
						<CheckboxGroup
							v-if="cont.obj.isCheck"
							size="large"
							class="positionColumn"
							v-model="cont.obj.valueArr"
						>
							<Row v-for="(item,ind) in cont.obj.items" :key="ind">
								<Checkbox class="checkboxItem" :value="item.label_value" :label="ind">
									<span>
										{{item.label_name}}
										&nbsp;
										<span
											v-text="item.scoreType=='add'?'+':'-'"
										></span>
										{{item.label_value}}分
									</span>
								</Checkbox>
					...
					<div class="selectStudentContainer" v-if="cont.ele=='truefalse'">
						...
						<RadioGroup size="large" v-model="cont.obj.value">
							<Radio v-for="(item,ind) in cont.obj.items" :key="ind" :label="item.label_value">
								<span>{{item.label_name}}</span>
							</Radio>
					...
					<div class="selectStudentContainer" v-if="cont.ele=='datepicker'">
						...
						<row>
							...
							<i-col span="8">
								<DatePicker
									v-show="cont.obj.chooseCheck[0]=='date'"
									format="yyyy-MM-dd"
									:value="cont.obj.valueDate"
									type="date"
									placeholder="选择日期"
									style="width: 200px"
									@on-change="cont.obj.valueDate=$event"
								></DatePicker>
							</i-col>
							<i-col span="8">
								<TimePicker
									v-show="cont.obj.chooseCheck[0]=='time'||cont.obj.chooseCheck[1]=='time'"
									:value="cont.obj.valueTime"
									type="time"
									placeholder="选择时间"
									style="width: 200px"
									@on-change="cont.obj.valueTime=$event"
								></TimePicker>
							</i-col>
						</row>
					</div>
					...
					<div class="selectStudentContainer" v-if="cont.ele=='uploads'">
						...
						<Row class="btn-view">
							<input class="files" :id="'files'+index" type="file" @change="addFile(cont.obj,index)">
...
<!--             <Row>
						...
						<Row style="margin-top:40px;">
							<Col span="16" class="slider-img">
								<img
									src="@/assets/icon-jian.png"
									@click="countRangeValue(-1, cont.obj)"
									width="40px"
									height="40px"
									alt
								>
					...
					<div class="selectStudentContainer" v-if="cont.ele=='imgcheck'">
						...
						<div class="imgCheckPreview">
							<CheckboxGroup
								v-model="cont.obj.value"
								style="width: 100%;display:flex;flex-wrap: wrap;flex-direction: row;justify-content:left;"
							>
				...
				<Row style="display: flex; justify-content: center">
					<Button
						type="primary"
						style="margin: 2rem auto; width: 8rem"
						@click="saveForm"
						:disabled="!isSave"
					>提 交</Button>
				</Row>
		...
		<Modal v-model="modalAdd" width="360">
			...
			<div style="text-align:center">
				<p>
					<img
						src="@/assets/tijiaochanggong_picture@.png"
						alt
						style="display:inline-block;height: 79px;width:79px;"
					>
				</p>
...
export default {
	...
	methods: {
		...
		addFile(res,index) {
			let self = this;
			let fileObj = document.getElementById("files"+index).files[0];
		...
		addFileImg(res,index) {
			let self = this;
			let fileObj = document.getElementById("fileImg"+index).files[0];
			let sizes = self.$api.onver(fileObj.size);
			self.$api.uploadFile("file/upload ", {}, fileObj, r => {
				res.uploadList.push({
					name: fileObj.name,
